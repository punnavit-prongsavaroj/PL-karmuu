...
```
  ...
  <header className="p-6 md:px-12 flex justify-between items-center relative z-20">
    <div>
      ...
      <div
        className="text-[10px] tracking-[4px] mt-1"
  ...
  <div className="flex-1 flex flex-col items-center justify-center text-center relative z-10 py-10">
    <div
      className="text-[10px] md:text-xs tracking-[6px] mb-4 uppercase"
    ...
    <p
      className="max-w-md mx-auto mb-12 text-sm md:text-base px-6"
      style={{ color: theme.textSecondary }}
    >
    ...
    <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4 w-full max-w-3xl px-4">
      {options.map((opt) => (
        <button
          ...
          className={[
            'relative p-6 md:p-10 rounded-2xl flex flex-col items-center justify-center text-center',
          ...
          style={{
            ...
            color:
              hovered === opt.id && opt.id === 'web' ? '#000' : theme.textPrimary,
            boxShadow:
              hovered === opt.id ? '0 10px 30px rgba(0,0,0,0.4)' : 'none',
          }}
        >
          <div
            ...
            style={{
              background:
                hovered === opt.id && opt.id === 'web'
                  ...
                  : 'rgba(201,168,76,0.1)',
              color:
                hovered === opt.id && opt.id === 'web' ? '#000' : theme.gold,
            }}
          >
            {opt.icon}
          </div>
          <div className="text-base md:text-xl font-bold mb-1">{opt.label}</div>
          <div className="text-[10px] md:text-xs opacity-70">{opt.sub}</div>
        </button>
      ))}
    </div>
  </div>
